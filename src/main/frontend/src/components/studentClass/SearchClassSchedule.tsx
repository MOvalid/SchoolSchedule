...
const SearchClassSchedule: React.FC = () => {
  ...
  return (
    <Box sx={{ my: 4 }}>
      <Typography variant="h5" gutterBottom>Wyszukaj klasę</Typography>
      ...
      {schedule.length > 0 && (
        <Box mt={4}>
          <Typography variant="h6" gutterBottom>Grafik klasy</Typography>
          <List>
            {schedule.map((slot) => (
              <Paper key={slot.id} sx={{ mb: 2, p: 2 }}>
                <ListItem>
                  <ListItemText
                    ...
                    secondary={
                      <>
                        Sala: {slot.room?.name || '–'}, Terapeuta: {slot.therapist?.firstName} {slot.therapist?.lastName}
                      </>
                    }
                  />
                </ListItem>
